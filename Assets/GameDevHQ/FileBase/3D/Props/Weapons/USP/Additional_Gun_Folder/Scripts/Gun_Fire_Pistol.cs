...
public class Gun_Fire_Pistol : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.Mouse0) && _weaponHolder._currentAmmoSecondary>0 && Time.time > _weaponHolder.reloadtime && !UIManager.Instance._menuOpen)
        {
            Debug.Log("Shoot gun");
            if (FullAuto == false)
            {
        ...
        if (Input.GetKeyUp(KeyCode.Mouse0) || _weaponHolder._currentAmmoSecondary <= 0)
        {
            if (FullAuto == true)
            {
        ...
        if(Input.GetKeyDown(KeyCode.R) &&  _weaponHolder._totalAmmoSecondary>0 )
        {
